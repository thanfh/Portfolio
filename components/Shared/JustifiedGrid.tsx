  ...
  return (
    <div ref={containerRef} className="flex flex-col w-full overflow-hidden" style={{ gap: `${gap}px` }}>
      {rows.map((row, rowIndex) => {
        ...
        // FIXED LOGIC: Reduced threshold to 1.35 to prevent massive expansion on last row
        const threshold = responsiveTargetHeight * 1.35; 
        
        const isTooTall = row.isLast && calculatedHeight > threshold;
        const finalHeight = isTooTall ? responsiveTargetHeight : calculatedHeight;
        
        // Always flex-grow unless it's strictly clamped
        const shouldGrow = !isTooTall;

        // Center the row if it's not growing to fill width (usually last row)
        const justifyClass = (!shouldGrow) ? 'justify-center' : 'justify-start';
        ...
        return (
          <div key={rowIndex} className={`flex w-full ${justifyClass}`} style={{ height: `${finalHeight}px`, gap: `${gap}px` }}>
            {row.items.map((wrapper: any, itemIndex: number) => {
              ...
              return (
                <div key={wrapper.item.id || itemIndex} style={itemStyle} className="relative">
